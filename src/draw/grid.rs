...
pub struct GridData {
    /// multiplier on `DEFAULT_CELL_SIZE`
    zoom_level: u32,
...
impl GridData {
    ...
    pub fn shift_x(&mut self, shift: i32) {
        let spacing = self.line_spacing();

        if let Some(spacing) = spacing {
            let spacing = spacing as i32;
            self.x = (((self.x + shift) % spacing) + spacing) % spacing;
        }
    }
    ...
    pub fn shift_y(&mut self, shift: i32) {
        ...
        if let Some(spacing) = spacing {
            let spacing = spacing as i32;
            self.y = (((self.y + shift) % spacing) + spacing) % spacing;
        }
    }
    ...
    pub fn draw(&self, canvas: &mut WindowCanvas, window: &WindowData) -> Result<(), String> {
        ...
        if let Some(spacing) = spacing {
            ...
            for i in 0..num_lines {
                let delta = self.x + (i * spacing) as i32;
            ...
            for i in 0..num_lines {
                let delta = self.y + (i * spacing) as i32;
